...
//                $scope.detailView = false;
                    ...
                    callback: function (result) { //setting callback
                        ...
                        if (result == true) {
                            $fhirApiServices.deleteResource($scope.smart, $scope.selectedResourceInstance, $scope.resourceTypeList, $scope.selectedResourceTypeConfig, $scope.notification)
                                .done(function (resourceTypeList, resourceTypeConfigIndex) {
                                    $scope.closeDetailView();
                                    updateView(resourceTypeList[resourceTypeConfigIndex], -1);
                                    modalProgress.dismiss();
                                });
                        }
                    }
                ...
                if (typeConfig.listPath) {
                    ...
                    if (isParticipant) {
                        if (collection.filter(function (e) {
                            return e.member.reference === reference.member.reference;
                        }).length === 0) {
                        ...
                        if (collection.filter(function (e) {
                            return e.reference === reference.resourceType + "/" + reference.id;
                        }).length === 0) {
            ...
            function updateView (resourceType, countOffset) {
                if (!countOffset) {
                    countOffset = 0;
                }
                if (typeof $scope.selectedResourceType.index === 'undefined' || resourceType.index === $scope.selectedResourceType.index) {
                    $scope.$apply(function () {
                        $scope.selectedResourceType = $scope.resourceTypeList[resourceType.index];
                        $scope.resourceTypeList[resourceType.index].count += countOffset;
                        rebuildResourceTable($scope.selectedResourceType.pageData);
                        $scope.resourcePages.pageCount = $scope.selectedResourceType.pageCount;
                ...
                $fhirApiServices.queryFhirVersion(smart)
                    .done(function (version) {
                        if (version === "1.0.2") {
                            ...
                            schemaVersion = 3;
                        } else if (version === "3.0.1" || version === "3.0.2") {
                            schemaVersion = 4;
                        } else if (version === "3.2.0" || version === "3.4.0" || version === "4.0.0" || version === "4.0.1") {
                            schemaVersion = 5;
                        }
